...
export default class PrestamoRepositoryPostgres implements PrestamoRepository{
    ...
    async postPrestarLibro(usuario:string, ejemplar: Number): Promise<Prestamo> {
        ...
        return this.getPrestamoByIdEjemplar(ejemplar);
    }
    
    async getPrestamoByIdEjemplar(ejemplar : Number){
    ...
    async getPrestamoByIdEjemplarCompleto(ejemplar : Number){
        const prestamoDB : any[] = await executeQuery(`SELECT * FROM public.prestamos WHERE ejemplar = '${ejemplar}'`);
        let prestamo : Prestamo = {
            ejemplar: prestamoDB[0].ejemplar,
            usuario : prestamoDB[0].usuario,
    ...
    async getPrestamosUsuario(token: any): Promise<Prestamo[]> {
        ...
        const prestamosDB : any[] = await executeQuery(`SELECT * FROM public.prestamos WHERE usuario = '${token}' AND fechadevolucion IS NULL`);
        for (const prestamoDB of prestamosDB) {
            let prestamo : Prestamo = {
                ejemplar: prestamoDB.ejemplar,
                fechaprestamo: prestamoDB.fechaprestamo
            }
    ...
    async putDevolverLibro(token: string | undefined, ejemplar: string): Promise<Prestamo> {
        ...
        return this.getPrestamoByIdEjemplarCompleto(Number(ejemplar));
    }

    async getPrestamosUsuarioSQL(token: any): Promise<Prestamo[]> {
